...
Antes sequer de se testar as medidas como descrito no capítulo \ref{ch:investigacao}, foi preciso criar um ambiente que permitisse acesso ao HCI.

A primeira tentativa de criar este ambiente foi em \emph{Windows Mobile 4}. Criar um ambiente neste Sistema Operativo (SO) era particularmente útil devido à disponibilidade de equipamentos com este SO. No entanto, a biblioteca disponível e respectiva documentação são complexas e incompletas. Por isso, não se conseguiu criar o ambiente necessário.
...
No final, a única alternativa era usar BlueZ. Foi nesta \foreign{framework} que assentou todo o trabalho na componente do Beacon. Tanto a \foreign{framework} como o código desenvolvido são em C.
...
No fim, a ideia que passou foi a de que, para controlo preciso do HCI, é preferível não delegar a \foreign{frameworks} o tratamento da comunicação, ganhando um maior controlo sobre todos os processos de BT. Para situações em que não é preciso tamanho controlo, o uso da \nname{framework} deverá ser uma vantagem.
...
\section{Alterando o Método de Localização}
